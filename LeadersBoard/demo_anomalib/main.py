...
def resolve_paths(config_path: Path, output: Path, config: DictConfig) -> None:
    """Normalize dataset/output paths and keep them under the demo tree."""
    # Ensure class_path/init_args structure exists first
    ensure_data_class_path(config)

    dataset_root = _resolve_dataset_root(config_path, config)
    _ensure_dataset_exists(dataset_root, base=config_path.parent)

    # Update init_args.root with resolved path
    if "init_args" not in config.data:
        config.data.init_args = {}
    config.data.init_args.root = str(dataset_root)
...
def _resolve_dataset_root(config_path: Path, config: DictConfig) -> Path:
    """Resolve dataset root path from config.

    Priority:
    1) config.data.path
    2) config.data.init_args.root
    3) config.data.init_args.path
    """
    base = config_path.parent
    data = config.data

    if "path" in data:
        return (base / data.path).expanduser()

    init_args = data.get("init_args", {}) or {}
    if "root" in init_args:
        return (base / str(init_args["root"])).expanduser()
    if "path" in init_args:
        return (base / str(init_args["path"])).expanduser()

    raise ValueError("data.path もしくは data.init_args.root の指定が必要です。")


def _maybe_extract_zip(dataset_root: Path, base: Path | None = None) -> None:
    """Extract a sibling zip (e.g., pcb1.zip) if dataset dir is missing."""
    if dataset_root.exists():
        return

    base = base or dataset_root.parent
    candidates: list[Path] = []

    # Typical case: dataset_root like <base>/pcb1/Data/Images -> zip at <base>/pcb1.zip
    try:
        rel = dataset_root.relative_to(base)
        if rel.parts:
            candidates.append(base / f"{rel.parts[0]}.zip")
    except ValueError:
        # dataset_root not under base; ignore
        pass

    # Fallback: zip next to the top-level dataset dir (two levels up from Images)
    if len(dataset_root.parents) >= 3:
        candidates.append(dataset_root.parents[2].with_suffix(".zip"))

    for zip_path in candidates:
        if zip_path.exists():
            LOGGER.info("Extracting dataset zip %s", zip_path)
            with zipfile.ZipFile(zip_path) as archive:
                archive.extractall(zip_path.parent)
            return


def _ensure_dataset_exists(dataset_root: Path, base: Path | None = None) -> None:
    """Require dataset to be pre-uploaded alongside submission; auto-extract zip if provided."""
    _maybe_extract_zip(dataset_root, base=base)
