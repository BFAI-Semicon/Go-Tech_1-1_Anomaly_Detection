...
def test_sequential_file_upload_integration(integration_context) -> None:
    """Test end-to-end sequential file upload functionality."""
    client = integration_context.client

    # Create initial submission with one file
    submission_id, status_code = _post_submission(client, _runner_script("sequential-test"))
    assert status_code == 201

    # Check initial file list
    files_response = client.get(f"/submissions/{submission_id}/files", headers=AUTH_HEADER)
    assert files_response.status_code == 200
    initial_files = files_response.json()["files"]
    assert len(initial_files) == 2  # main.py and config.yaml from initial submission
    initial_filenames = {f["filename"] for f in initial_files}
    assert initial_filenames == {"main.py", "config.yaml"}

    # Add additional files sequentially
    ...
    # Add custom-config.yaml
    custom_config_file = io.BytesIO(custom_config_content.encode())
    custom_config_file.name = "custom-config.yaml"
    add_file_response = client.post(
        f"/submissions/{submission_id}/files",
        headers=AUTH_HEADER,
        files={"file": ("custom-config.yaml", custom_config_file, "application/x-yaml")},
    )
    assert add_file_response.status_code == 201
    assert add_file_response.json()["filename"] == "custom-config.yaml"

    # Add data file (using .zip extension which is allowed)
    ...
    assert add_data_response.status_code == 201
    assert add_data_response.json()["filename"] == "data.zip"

    # Check updated file list
    files_response = client.get(f"/submissions/{submission_id}/files", headers=AUTH_HEADER)
    assert files_response.status_code == 200
    final_files = files_response.json()["files"]
    assert len(final_files) == 4

    filenames = {f["filename"] for f in final_files}
    assert filenames == {"main.py", "config.yaml", "custom-config.yaml", "data.zip"}
    ...
    # Now submit job with complete submission
    job_response = client.post(
        "/jobs",
        headers=AUTH_HEADER,
        json={"submission_id": submission_id, "config": {"lr": 0.01}},
    ...
    # Execute job
    job_payload = integration_context.queue_adapter.dequeue()
